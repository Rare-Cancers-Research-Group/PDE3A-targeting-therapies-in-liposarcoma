...
#healthy.genes <- top.500.genes.per.tissue %>% filter(grouping_class==15) %>% group_by(ensg_index) %>% summarise(nn=n()) %>% filter(nn > 1)

#cancer.genes <- top.500.genes.per.tissue %>% filter(grouping_class==20) %>% group_by(ensg_index) %>% summarise(nn=n()) %>% filter(nn > 3)

#excl.list <- tibble(ensg_index=unique(pull(healthy.genes,ensg_index), pull(cancer.genes, ensg_index)))
